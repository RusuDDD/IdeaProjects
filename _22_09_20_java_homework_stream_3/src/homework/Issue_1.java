...
// отфильтровать студентов age>22 а вывести на экран только одного [use  limit ]
        ...
        System.out.println(justOne(studentList));

    }

    public static List<Student> justOne(List<Student> list) {
        return list.stream()
                .filter(s -> s.getAge() > 22).limit(1).collect(Collectors.toList());
    }
}
